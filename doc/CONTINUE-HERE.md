...
早期 proportion-v3 等候选仍为拒绝状态；最终主角运行图集来自后续分方向平台帧和整帧装配，不能混淆。用户随后撤销旧 NPC 签收：其四宫格素材在真实画面中比主角显小且风格不同。五名 NPC 已按主角 accepted 单帧标准逐方向经平台重生并装配，等比感知尺寸调整为 114/128；`_qa/npc-scale-review.mjs` 已保存五组主角相邻同屏证据。Daniel 的长腿走路候选被拒绝，未进入运行图集。技能已增加整组演员的可见宽度、头部、肩宽、像素密度和逐人同屏门禁。当前待办是最终回归、发布扫描、同提交双部署、生产截图与 `release.json` 更新；生产地址仍显示上一版。
