...
public/art 与 public/audio 是准入后的运行资源；materials 内保留选定原图和服务任务记录。scripts/media-plan.json 和 generate-assets.mjs 可重用稳定请求，prepare-art.py 做裁切、连通背景处理和等比图集装配。doc/media-provenance.json 对应每个运行资源。
...
四房间640×640；人物占地14×10，渲染框80×80。共享布局生成碰撞和接近候选，BFS路线逐像素检查边，108世界像素/秒移动。实际距离驱动行走与25像素一步音。分析师在44像素岗位范围巡游，其余角色值守；接近105像素停步转向。人物、家具按脚底深度排序，门洞补片先画，底墙最后画。
