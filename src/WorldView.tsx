...
export function WorldView({journeyId,scene,start,locale,paused,known,onNear,onPosition,handle}:{journeyId:string;scene:SceneId;start:Point;locale:Locale;paused:boolean;known:string[];onNear:(entity:Entity|null)=>void;onPosition:(point:Point)=>void;handle:{current:WorldHandle|null}}){
 ...
 useEffect(()=>{
  ...
  const space=createRpgSpace({world,host:mount,scene,position:start,speed:108,stride:52,sheet:runtimeHero,spritesheets:runtimeSheets,mapEvents,prepareScene,controlsBlocked:()=>latest.current.paused,walkable:dynamicWalkable,findPath:dynamicPath,
   ...
   onFrame:(dt,position,activeScene,isPaused)=>{if(stopped||activeScene!==latest.current.scene)return;const room=rooms[activeScene as SceneId],active=residents.current[activeScene as SceneId];
    ...
    const qa=host.current?.querySelector('output[data-wall-qa]');if(qa)qa.textContent=`x ${Math.round(position.x)} · y ${Math.round(position.y)}`;
 ...
 const wallReview=import.meta.env.DEV&&new URLSearchParams(location.search).has('wallReview');
 return <div ref={host} className="bc-world"><div id="rpg" aria-label={tx(rooms[scene].title,locale)}/>{!ready&&<div className="bc-world-loading">{error?<div><p>{tx(['场景未能载入','Scene unavailable'],locale)}</p><button onClick={()=>location.reload()}>{tx(['重新载入','Retry'],locale)}</button></div>:tx(['正在抵达…','Arriving…'],locale)}</div>}{wallReview&&<details style={{position:'absolute',top:150,right:4,zIndex:20,background:'#203b40',padding:4}}><summary>QA walls</summary>{[['NW',45,140],['NE',565,140],['SW',80,538],['SE',550,538],['Center',310,420]].map(([name,x,y])=><button key={name} onClick={event=>{handle.current?.go({x:Number(x),y:Number(y)});event.currentTarget.closest('details')?.removeAttribute('open')}}>{String(name)}</button>)}<output data-wall-qa/>{sideLeafBodies(scene).length>0&&<><button onClick={()=>{const leaf=sideLeafBodies(scene)[0];handle.current?.go({x:leaf.x+16,y:leaf.y+18})}}>Below leaf</button><button onClick={()=>{handle.current?.move(0,-1);setTimeout(()=>handle.current?.move(0,0),1000)}}>Push north 1s</button><button onClick={()=>{const leaf=sideLeafBodies(scene)[0];handle.current?.go({x:leaf.x+16,y:leaf.y-22})}}>Around leaf</button></>}</details>}<div className="bc-access-targets">{rooms[scene].entities.map(entity=><button key={entity.id} data-entity={entity.id} data-person={Boolean(entity.person)} onClick={event=>{event.stopPropagation();handle.current?.approach(entity)}}>{entity.person?tx(known.includes(entity.person)?people[entity.person].name:people[entity.person].unknown,locale):tx(entity.label,locale)}</button>)}</div></div>;
}
