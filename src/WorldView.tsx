...
export function WorldView({journeyId,scene,start,locale,paused,known,onNear,onPosition,handle}:{journeyId:string;scene:SceneId;start:Point;locale:Locale;paused:boolean;known:string[];onNear:(entity:Entity|null)=>void;onPosition:(point:Point)=>void;handle:{current:WorldHandle|null}}){
 ...
 return <div ref={host} className="bc-world"><div id="rpg" aria-label={tx(rooms[scene].title,locale)}/>{!ready&&<div className="bc-world-loading">{error?<div><p>{tx(['场景未能载入','Scene unavailable'],locale)}</p><button onClick={()=>location.reload()}>{tx(['重新载入','Retry'],locale)}</button></div>:tx(['正在抵达…','Arriving…'],locale)}</div>}{wallReview&&<details style={{position:'absolute',top:150,right:4,zIndex:20,background:'#203b40',padding:4}}><summary>QA walls</summary>{[['NW',45,110],['NE',565,110],['SW',80,538],['SE',550,538],['Center',310,420]].map(([name,x,y])=><button key={name} onClick={event=>{handle.current?.go({x:Number(x),y:Number(y)});event.currentTarget.closest('details')?.removeAttribute('open')}}>{String(name)}</button>)}</details>}<div className="bc-access-targets">{rooms[scene].entities.map(entity=><button key={entity.id} data-entity={entity.id} data-person={Boolean(entity.person)} onClick={event=>{event.stopPropagation();handle.current?.approach(entity)}}>{entity.person?tx(known.includes(entity.person)?people[entity.person].name:people[entity.person].unknown,locale):tx(entity.label,locale)}</button>)}</div></div>;
}
