...
export function dialogueMessages(j:Journey,person:PersonId,input:string,locale:Locale){
 ...
 const context={name:t(people[person].name),role:t(people[person].role),introduction:t(people[person].intro),availableTopics:topics[person].filter(q=>!q.requires||q.requires.every(r=>has(j,r))).map(q=>({question:t(q.label),answer:t(q.reply)})),discoveredSources:records.filter(r=>has(j,r.id)).map(r=>({title:t(r.title),summary:t(r.summary)}))};
